...
<section class="<?php echo esc_attr($header['class']); ?>"<?php echo $header['style']; ?>>
    <div class="fp-container">
        <h1>Properties in <?php echo esc_html($term->name); ?></h1>
        ...
        <?php if (have_posts()) : ?>
            ...
            <?php if ($wp_query->max_num_pages > 1) : ?>
                <div class="fp-load-more">
                    <button type="button" id="fp-load-more-btn" class="fp-btn fp-btn--primary" data-target="fp-property-grid" data-max-pages="<?php echo (int) $wp_query->max_num_pages; ?>" data-taxonomy="location" data-term="<?php echo esc_attr($term->slug); ?>">
                        Load More
                    </button>
                </div>
            <?php endif; ?>
        <?php else : ?>
            <p>No properties found in this location yet.</p>
        <?php endif; ?>
    </div>
</section>
